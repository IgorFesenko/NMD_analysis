...
out_file = r'D:\Google Диск\NMD moss\Lloyd\smg1_assembled_transcripts_v1.6_broadNMDtargets_translation_withAUG.txt'
    ...
    for i in range(3):
        ...
        for pep in lst_translation[i].split('*'):
            if 'M' in pep:
                ...
                if len(new_seq) >= 30:
                    pep_numb += 1
                    records.append(SeqRecord(seq=new_seq, id=record.id + '_' + frame_number[i]+'peptide' + str(pep_numb), description=""))
